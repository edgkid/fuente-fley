...
exports.add_languages_detail = function (req, res) {
    if (typeof req.session.userid != 'undefined') {
        ...
        language.save().then((language) => { 
            res.json({
                success: true,
                message: req.__(admin_messages.success_message_add_language),
                data: language
            })
        }, (err) => {
            console.log(err);
...
exports.update_languages_detail = function (req, res) {
    if (typeof req.session.userid != 'undefined') {
        var data = req.body;
        Language.findByIdAndUpdate(req.body.id, data).then(() => {
            res.json({
                success: true,
                message: req.__(admin_messages.success_message_update),
                data: { ...req.body }
            })
        });
    } else {
