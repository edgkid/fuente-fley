    ...
    Country.find({}).then((country_list) => {
        if (typeof req.session.userid != 'undefined') {
            ...
            Trip_history.aggregate([trip_condition, trip_filter, country_filter, city_filter, trip_group_condition, lookup, search, count]).then((array) => {
                ...
                if (array.length == 0) {
                    array = [];
                    res.render('daily_earning', {detail: array, 'current_page': 1, country_list: country_list, city_list: city_list, type: req.body.type, 'pages': 0, 'next': 1, 'pre': 0, moment: moment, start_date: start_date, end_date: end_date,sort_field, sort_order, search_item, search_value, filter_start_date, filter_end_date});
                        ...
                        if (array.length == 0) {
                            ...
                            Trip_history.aggregate([trip_condition, trip_filter, country_filter, city_filter, trip_group_condition_total]).then((trip_total) => {
                                ...
                                if (trip_total.length == 0) {
                                    ...
                                    res.render('daily_earning', {detail: array, 'current_page': page, trip_total: trip_total, country_list: country_list, city_list: city_list, type: req.body.type, 'pages': pages, 'next': next, 'pre': pre, moment: moment, start_date: start_date, end_date: end_date,sort_field, sort_order, search_item, search_value, filter_start_date, filter_end_date});
...
exports.generate_daily_earning_excel = function (req, res) {
    if (req.body.page == undefined)
    {
        page = 0;
        next = 1;
                                    ...
                                    Trip_history.aggregate([trip_condition, trip_filter, country_filter, city_filter, trip_group_condition_total]).then(() => {
                                        ...
                                        array.forEach(function (data, index) {
                                            ...
                                            if (index == array.length - 1) {
                                                wb.write('data/xlsheet/' + time + '_daily_earning.xlsx', function (err) {
                                                    if (err) {
                                                        console.error(err);
                                                    } else {
                                                        ...
                                                        res.json(url);
                                                        setTimeout(function () {
                                                            fs.unlink('data/xlsheet/' + time + '_daily_earning.xlsx', function () {
                                                            });
                                                        }, 10000)
                                                    }
                                                });
...
exports.statement_provider_daily_earning = function (req, res) {
