...
exports.type_capacity_list = async function (req, res) {
    ...
    array = await Type_Capacity.aggregate([search, filter, count]);
    if (!array || array.length == 0) {
        array = [];
        res.render('type_capacity', {
            detail: array,
            current_page: 1,
            pages: 0,
            next: 1,
    ...
    array = await Type_Capacity.aggregate([search, filter, sort, skip, limit])
    res.render('type_capacity', {
        detail: array,
...
exports.generate_type_capacity_excel = async function (req, res) {
    if (typeof req.session.userid == 'undefined') {
        res.redirect('/admin');
        return;
    }
    ...
    array = await Type_Capacity.aggregate([search, filter, sort])
    let date = new Date()
...
exports.edit = async function (req, res) {
    ...
    let id = req.body.id;
    let type_capacity = await Type_Capacity.findById(id);
    res.render('type_capacity_detail_edit', { data: type_capacity, moment: moment, timezone_for_display_date: setting_detail.timezone_for_display_date });
    return;
};
...
exports.update = async function (req, res) {
    ...
    let id = req.body.id;
    await Type_Capacity.findByIdAndUpdate(id, req.body)
    res.redirect('/type_capacity');
    return;
};
...
exports.act = async function (req, res) {
    ...
    await Type_Capacity.findByIdAndUpdate(id, { state: change });
    res.redirect('/type_capacity');
    return;
};


exports.add_type_capacity_form = function (req, res) {
    if (typeof req.session.userid == "undefined") {
        res.redirect('/admin');
        return;
    }
    res.render('type_capacity_detail_edit');
    return;
};

exports.add_type_capacity = async function (req, res) {
    if (typeof req.session.userid == "undefined") {
        res.redirect('/admin');
        return;
    }
    let capacity_without_seperator = req.body.capacity_name.split('.').join("");
    let capacity_with_seperator = capacity_without_seperator.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".");
    let is_capacity_exist = await Type_Capacity.findOne({ $or: [{capacity_name: capacity_without_seperator},{capacity_name: capacity_with_seperator},{capacity_name: req.body.capacity_name}], unit: req.body.unit})
    if(is_capacity_exist){
        message = admin_messages.error_message_same_capacity_error;
        res.redirect('/type_capacity');
        return
    }
    var type_capacity = new Type_Capacity({
        capacity_name: req.body.capacity_name,
        unit: req.body.unit,
        value: req.body.value,
        state: 1
    });

    await type_capacity.save()
    res.redirect("/type_capacity");
    return;
};