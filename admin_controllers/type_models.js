let myUtils = require('../controllers/utils');
...
exports.type_model_list = async function (req, res) {
    ...
    array = await Type_Models.aggregate([search, filter, count]);
    if (!array || array.length == 0) {
        array = [];
        res.render('type_models', {
            detail: array,
            current_page: 1,
            pages: 0,
            next: 1,
    ...
    array = await Type_Models.aggregate([search, filter, sort, skip, limit])
    res.render('type_models', {
        detail: array,
...
exports.generate_type_model_excel = async function (req, res) {
    if (typeof req.session.userid == 'undefined') {
        res.redirect('/admin');
        return;
    }
    ...
    array = await Type_Models.aggregate([search, filter, sort])
    let date = new Date()
...
exports.edit = async function (req, res) {
    ...
    let id = req.body.id;
    let type_model = await Type_Models.findById(id) 
    let services = await Type_Services.find({ state: 1 }).lean(); 
    res.render('type_model_detail_edit', { data: type_model, moment: moment, type_services: services, timezone_for_display_date: setting_detail.timezone_for_display_date });
    return;
};
...
exports.update = function (req, res) {
    ...
    if(req.body.sequence != ""){
     ...
     Type_Models.findByIdAndUpdate(id, req.body).then((model_detail)=>{
        var files_details = req.files;

        if (files_details != null && files_details != 'undefined' && files_details.length > 0) {
            ...
            myUtils.deleteImageFromFolder(model_detail.model_image_url, 11);
            var image_name = model_detail._id + myUtils.tokenGenerator(4);
            var url = myUtils.getImageFolderPath(req, 11) + image_name + '.png';
            model_detail.model_image_url = url;
            myUtils.saveImageFromBrowser(file_data.path, image_name + '.png', 11);
            model_detail.save().then(() => { 
            }, (err) => {
                console.log(err)
            });
...
exports.act = async function (req, res) {
    ...
    await Type_Models.findByIdAndUpdate(id, { state: change });
    res.redirect('/type_models');
    return;
};


exports.add_type_model_form = async function (req, res) {
    if (typeof req.session.userid == "undefined") {
        res.redirect('/admin');
        return;
    }
    let services = await Type_Services.find({ state: 1 }).lean(); 
    res.render('type_model_detail_edit', {type_services: services});
    return;
};

exports.add_type_model = async function (req, res) {
    if (typeof req.session.userid == "undefined") {
        res.redirect('/admin');
        return;
    }
    ...
    var type_model = new Type_Models({
    ...
    if (req.files.length) {
        ...
        var image_name = type_model._id + myUtils.tokenGenerator(4);
        var url = myUtils.getImageFolderPath(req, 11) + image_name + '.png';
        type_model.model_image_url = url;
        myUtils.saveImageFromBrowser(req.files[0].path, image_name + '.png', 11);
