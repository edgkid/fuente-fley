...
exports.service_specification_list = async function (req, res) {
    ...
    array = await Service_Specifications.aggregate([search, filter, count]);
    if (!array || array.length == 0) {
        array = [];
        res.render('service_specifications', {
            detail: array,
            current_page: 1,
            pages: 0,
            next: 1,
    ...
    array = await Service_Specifications.aggregate([search, filter, sort, skip, limit])
    res.render('service_specifications', {
        detail: array,
...
exports.generate_service_specification_excel = async function (req, res) {
    if (typeof req.session.userid == 'undefined') {
        res.redirect('/admin');
        return;
    }
    ...
    array = await Service_Specifications.aggregate([search, filter, sort])
    let date = new Date()
...
exports.edit = async function (req, res) {
    ...
    let id = req.body.id;
    let service_specification = await Service_Specifications.findById(id);
    res.render('service_specification_detail_edit', { data: service_specification, moment: moment, timezone_for_display_date: setting_detail.timezone_for_display_date });
    return;
};
...
exports.update = async function (req, res) {
    ...
    let id = req.body.id;
    await Service_Specifications.findByIdAndUpdate(id, req.body)
    res.redirect('/service_specifications');
    return;
};
...
exports.act = async function (req, res) {
    ...
    await Service_Specifications.findByIdAndUpdate(id, { state: change });
    res.redirect('/service_specifications');
    return;
};


exports.add_service_specification_form = function (req, res) {
    if (typeof req.session.userid == "undefined") {
        res.redirect('/admin');
        return;
    }
    res.render('service_specification_detail_edit');
    return;
};

exports.add_service_specification = async function (req, res) {
    if (typeof req.session.userid == "undefined") {
        res.redirect('/admin');
        return;
    }
    var service_specification = new Service_Specifications({
        specification_name: req.body.specification_name,
        specification_note: req.body.specification_note,
        state: 1
    });

    await service_specification.save()
    res.redirect("/service_specifications");
    return;
};